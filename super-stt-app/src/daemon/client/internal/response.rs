...
/// Return `resp` when `status == "success"`, otherwise the daemon's
/// `message` (or `"<context> failed"` when absent) as the `Err` string.
pub(crate) fn require_success(
    ...
    if resp.status == "success" {
        ...
        Err(resp.message.unwrap_or_else(|| format!("{context} failed")))
    }
}

/// `require_success`, discarding the body — for writes that only need
/// success/failure.
pub(crate) fn require_unit(resp: DaemonResponse, context: &str) -> Result<(), String> {
...
pub(crate) fn require_message(resp: DaemonResponse, context: &str) -> Result<String, String> {
