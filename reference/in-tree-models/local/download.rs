...
///
/// Used to provision a subprocess backend's per-backend model directory (the
/// manifest `dest`). Skips files already present (non-zero size). Unlike the
/// HF-cache layout above, this writes plain `dest_dir/<filename>` files so a
/// sandboxed backend can load them directly.
///
/// # Errors
///
/// Returns an error on network/IO failure or a non-success HTTP status.
pub async fn download_files_to_dir(
    ...
    for filename in files {
        ...
        let url = get_hf_url(repo, revision, filename);
