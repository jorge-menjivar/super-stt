...
impl SuperSTTDaemon {
    ...
    async fn emit_listening_started_dbus(&self, write_mode: bool) {
        ...
        let client_id = "daemon_recorder".to_string();
    ...
    async fn finalize_recording_session(
        ...
        // Widget bus — paired with the D-Bus emit below so subscribers
        // on either transport see the same fact in the same order.
        self.events
