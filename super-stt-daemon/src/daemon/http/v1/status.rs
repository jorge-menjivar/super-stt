...
//!
//! A summary for a client with the `status` scope alone: the loaded model, the
//! device under it, and whether a recording is in flight. The per-stage detail
//! behind it is `settings`-scoped, in [`super::pipeline`].
