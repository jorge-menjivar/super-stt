...
//! `/language` (global) + `/active_model/language` (per-model) settings routes.
...
pub(crate) async fn get_active_model_language(State(s): State<AppState>) -> impl IntoResponse {
    dispatch_command(&s.daemon, "get_active_model_language", None).await
}

pub(crate) async fn set_active_model_language(
    State(s): State<AppState>,
    axum::Json(body): axum::Json<SetLanguageBody>,
) -> impl IntoResponse {
    dispatch_command(
...
pub(crate) async fn clear_active_model_language(State(s): State<AppState>) -> impl IntoResponse {
    dispatch_command(&s.daemon, "clear_active_model_language", None).await
}
