...
impl SuperSTTDaemon {
    ...
    /// Stop audio monitoring if no `audio_level` subscribers remain.
    /// Audio level streams via the notification subscribe path
    /// (`/api/events`-style) and the widget HTTP/SSE bus; both are
    /// counted as live consumers via `has_audio_level_clients`.
    pub async fn ensure_audio_monitoring_stopped(&self) {
        if !self.has_audio_level_clients() {
    ...
    /// Whether any consumer is currently subscribed to the
    /// `audio_level` stream. Today the only counted source is the
    /// notification manager (legacy subscribe path); the widget
    /// HTTP/SSE path publishes unconditionally — the daemon never gates
    /// capture on HTTP subscribers because publishes with no
    /// subscribers are a cheap drop.
    #[must_use]
    pub fn has_audio_level_clients(&self) -> bool {
        self.notification_manager
            .has_subscribers_for_event("audio_level")
    }

    /// Broadcast recording state change to all widget HTTP/SSE
