...
//! producer and the `/registry/backend/list` leaf types); the policy over it
//! — the `min_client` soft-floor check and the unsafe-path backend filter —
//! is `super_engine_daemon::registry::index`, shared with Super TTS.
