...
//! Super STT's secrets in the system keyring: each backend's API credentials
//! and the daemon's session tokens, under the `super-stt` service. The store
...
/// Super STT's keyring: the system one, or the in-memory store under test.
#[must_use]
pub fn keyring() -> Keyring {
    if cfg!(test) {
        Keyring::in_memory(&SUPER_STT)
    } else {
        Keyring::system(&SUPER_STT)
    }
}
...
#[cfg(test)]
mod tests {
    /// The sessions account is the one Super STT has always used. A change
    ...
    #[test]
    fn the_sessions_account_is_the_one_super_stt_shipped() {
        assert_eq!(super::keyring().sessions_account(), "stt-sessions");
    }
}
