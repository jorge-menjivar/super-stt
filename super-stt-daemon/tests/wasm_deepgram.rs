...
//! End-to-end harness for the Deepgram WASM backend: loads the component,
...
/// Happy path: the component shapes the Deepgram request (Token auth, raw
/// WAV body, model in the query string), the host permits the allowlisted
/// upstream, and the transcription comes back from
...
async fn live_deepgram() {
    ...
    let key =
        std::env::var("DEEPGRAM_API_KEY").expect("DEEPGRAM_API_KEY must be set for live test");
    let audio_path =
