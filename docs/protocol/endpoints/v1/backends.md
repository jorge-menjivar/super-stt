  ...
  "backends": [
    {
      ...
      "name":   "OpenAI",
      "version": "0.1.1",               // the installed backend's own version
...
| `…[].version`     | string           | The installed backend's `[backend].version`, read from the `backend.toml` on disk. This is what is installed, not what is published: it is the only version available for a backend the registry does not list — one imported from a directory or installed from an arbitrary repo — and it is authoritative for the rest, since the registry reports what a release offers rather than what this machine has. Empty only for a backend installed before the field existed. |
